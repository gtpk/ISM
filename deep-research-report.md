...
둘째, ISM과 요약은 모두 filler를 제거하므로 둘 사이 비교는 이 artifact와 분리되지만, 현재 N으로는
확정할 수 없다.

따라서 현 시점의 잠정 결론은 다음과 같다. **dev smoke는 ISM의 차별점이 순수한 토큰 효율보다는
검사 가능성(inspectability)과 개입 가능성(intervention)에 있을 가능성을 시사한다**(6.1에서 사전
의미내용·심볼 구조의 기능적 사용이 관찰된 점과 정합적). Fixed-Budget 효율 비교의 확정은 (a) 더 큰
N과 (b) filler 설계가 Full Context를 부당하게 깎지 않는지에 대한 통제 후로 미룬다.
